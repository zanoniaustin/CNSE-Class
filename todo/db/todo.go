...
func New(dbFile string) (*ToDo, error) {
	...
	//Now that we know the file exists, at the minimum we have
...
func (t *ToDo) RestoreDB() error {
	...
	if err != nil {
		return errors.New("Error opening backupFile")
	}
	...
	bytes, err := io.Copy(dbFile, backupFile)
	if err != nil {
		return errors.New("Error copying backupFile to dbFile")
	}

	fmt.Printf("Successfully copied over %v number of bytes", bytes)
...
func (t *ToDo) AddItem(item ToDoItem) error {
	...
	err := t.loadDB()
	if err != nil {
		return errors.New("Error loading dbFile")
	}

	_, ok := t.toDoMap[item.Id]
	if ok {
		return errors.New("Id already exists inside db file")
	}
	...
	error := t.saveDB()
	if error != nil {
		return errors.New("Problem deleting from database")
	}

	println("Successfully added item with id ", item.Id)
...
func (t *ToDo) DeleteItem(id int) error {
	...
	_, ok := t.toDoMap[id]
	if !ok {
		return errors.New("Id does not match any existing data id")
	}
...
func (t *ToDo) UpdateItem(item ToDoItem) error {
	...
	println("Successfully updated item with id ", item.Id)
...
func (t *ToDo) GetItem(id int) (ToDoItem, error) {
	...
	err := t.loadDB()
	if err != nil {
		return ToDoItem{}, errors.New("Error loading dbFile")
	}
	...
	return ToDoItem{}, errors.New("Id provided does not exist in database")
}
...
func (t *ToDo) GetAllItems() ([]ToDoItem, error) {
	...
	err := t.loadDB()
	if err != nil {
		return nil, errors.New("Error loading dbFile")
	}
...
func (t *ToDo) ChangeItemDoneStatus(id int, value bool) error {
	...
	if item.IsDone == value {
		return errors.New("Item status provided is already set")
	}

	item.IsDone = value

	error := t.UpdateItem(item)
	if error != nil {
		return error
	}
